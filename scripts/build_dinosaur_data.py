...
PBDB_TAXA_URL = (
    ...
    "?base_name=Dinosauria%5EAves&rank=species&show=app,parent,size,class&limit=all"
)
PBDB_OCCS_URL = (
    "https://paleobiodb.org/data1.2/occs/list.csv"
    "?base_name=Dinosauria%5EAves&taxon_reso=species&show=coords,class,time&limit=all"
)
...
def build_type_phrase(
    ...
    if family:
        return f"{family_descriptor(family)} dinosaur"
    if major_clade:
...
def build_image_record(species: dict, enrichment: dict | None) -> dict:
    ...
    remote_url = clean_whitespace((enrichment or {}).get("imageUrl"))
    match = species["size"].get("match")

    if remote_url and match == "exact":
        kind = "exact"
    elif remote_url and match == "genus_proxy":
        kind = "genus_proxy"
    else:
        kind = "fallback"
    ...
    return {
        "kind": kind,
        "detailUrl": remote_url or fallback_url,
        "listUrl": remote_url or fallback_url,
        "fallbackUrl": fallback_url,
        "alt": f"Illustration of {species['scientificName']}",
        "sourceDataset": (enrichment or {}).get("sourceDataset")
        if remote_url
        else "Local silhouette fallback",
        "sourceUrl": remote_url or fallback_url,
    }
...
def build_species_database(
    ...
    for row in occurrence_rows:
        ...
        if key not in species_by_id:
            ...
            species_by_id[key] = {
                ...
                "majorClade": clean_taxon(row.get("class")),
                "order": clean_taxon(row.get("order")),
                "taxonomyPath": None,
                "temporalRange": {
    ...
    return {
        "metadata": {
            ...
            "notes": [
                "Species are sourced from the PBDB Dinosauria^Aves taxonomy feed.",
                ...
                "Size values are enriched from the World Dinosaur Dataset when an exact species match or genus proxy is available.",
                ...
                "Each species record includes an image reference, using exact or genus-proxy enrichment artwork when available and a local silhouette fallback otherwise.",
            ],
            "sources": [
                {
                    "name": "Paleobiology Database taxonomic names API",
                    "url": PBDB_TAXA_URL,
                    "role": "Accepted Dinosauria^Aves species and taxonomy",
                },
                {
                    "name": "Paleobiology Database fossil occurrences API",
                    "url": PBDB_OCCS_URL,
                    "role": "Fossil occurrence coordinates and interval data",
                },
                {
                    "name": "World Dinosaur Dataset",
                    "url": WORLD_DINO_URL,
                    "role": "Length, weight, and type enrichment when available",
                },
            ],
        },
...
def main() -> None:
    ...
    taxa_rows = parse_csv(
        load_source_text(raw_dir / "pbdb-species-taxa.csv", PBDB_TAXA_URL)
    )
    occurrence_rows = parse_csv(
        load_source_text(raw_dir / "pbdb-species-occurrences.csv", PBDB_OCCS_URL)
    )
    size_rows = parse_csv(
