...
function getFilteredSpecies() {
  ...
  const filtered = species.filter((item) => {
    if (state.clade !== "all" && item.majorClade !== state.clade) return false;
...
function renderSpeciesList(filtered) {
  ...
  if (!filtered.length) {
    elements.speciesCaption.textContent =
      "No species match the current filters. Try widening the search or clearing a taxonomy filter.";
    elements.speciesList.innerHTML = `
      <div class="empty-state">
        No dinosaurs match this filter set right now.
      </div>
    `;
    return;
  }
          ...
          <div class="species-item-head">
            ...
            <div>
              ...
              <div class="species-meta">
                <div class="meta-row">
                  <span>Family</span>
                  <span>${escapeHtml(item.family || "Unspecified")}</span>
                </div>
                <div class="meta-row">
                  <span>Age</span>
...
function renderDetailPanel(filtered, selected) {
  ...
  if (!selected) {
    ...
    elements.detailPanel.innerHTML = `
      ...
      <p class="detail-copy">
        Choose a dinosaur species from the list to highlight its fossil localities. Without a selection,
      ...
      <div class="chips">${familyChips}</div>
    `;
    return;
  }
...
function renderSourcesPanel() {
  ...
  elements.sourcesPanel.innerHTML = `
    ...
    <p>
      The atlas currently includes ${escapeHtml(formatNumber(data.metadata.speciesCount))} accepted species from the PBDB <code>Dinosauria^Aves</code> feed,
...
function attachControlEvents() {
  ...
  elements.cladeFilter.addEventListener("change", (event) => {
    state.clade = event.target.value;
    render();
  });
